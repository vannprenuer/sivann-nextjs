...
// Paginated posts for the full blog archive (/blog/all) -- returns the
// requested page plus the total page count (from WordPress's
// X-WP-TotalPages response header) so the archive page can render proper
// numbered pagination instead of loading everything at once.
export async function getPostsPaged(page = 1, perPage = 20) {
  if (API_BASE) {
    try {
      const res = await fetch(`${API_BASE}/wp/v2/posts?per_page=${perPage}&page=${page}&_embed`, {
      ...
      if (res.ok) {
        ...
        return { posts: data.map(mapPost), totalPages, total };
      }
    } catch (e) {
      console.warn('WP API fetch failed (getPostsPaged), falling back to seed content:', e.message);
    }
  }
  const totalPages = Math.max(1, Math.ceil(seed.posts.length / perPage));
  const start = (page - 1) * perPage;
  return { posts: seed.posts.slice(start, start + perPage), totalPages, total: seed.posts.length };
}
